...
def run(config,ls,_k,_p,_t):
    ...
    for idx in range(_t):
        ...
        for i in range(_p):
            ...
            print("\n[INFO] train, val")
            #Alterado aa função na lib para utils_lib.load_data_ttv
            train, val = utils_lib.load_data_train(_path_data,
                              K=k,
...
if __name__=="__main__":
    ...
    config={'epochs':100,
            'alpha': 1e-5,
             'learning_rate':1e-4,
             'img_size': 224,
             'path_test': '1_Fine_tuned/DFT_REPORTS/',
             'test_sim': '0_CLASS_DFT_VISTAS.xlsx',
             'algoritmo':'1_Fine_tuned/OrigModel_DFT_K10_RC_xlsx_190624.py'
             }
    ls=2
    _k=1
    _p=10
